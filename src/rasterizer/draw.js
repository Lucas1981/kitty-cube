...
export function drawObject(object, camera, sun, cxData) {
  ...
  for (let j = 0; j < object.poly.length; j++) {
    ...
    const distToSun = vectorLength(subVector(sun, scaleVector(centroid, 1 / 3)));
...
export function drawObjectGouraudTexture(
  ...
  for (let j = 0; j < object.poly.length; j++) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      let point = dotProduct(
        object.poly[j].surfaceNormal,
        normalizeVector(d)
      );
      if (point < 0) point = 0;
      ...
      dc[i].push(sc[1]);
      dc[i].push(sc[2]);
      dc[i].push(object.uv[j % 2][i][0]);
      dc[i].push(object.uv[j % 2][i][1]);
